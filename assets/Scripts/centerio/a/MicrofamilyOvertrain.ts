/**
 * ⚠️自动生成文件（工具生成/更新会覆盖除 PLANIST_MACROCOMBINE 之外的内容）
 * ⚠️请勿手动修改除 PLANIST_MACROCOMBINE 之外的内容。
 *
 * 📌埋点事件手动配置，仅可修改配置 PLANIST_MACROCOMBINE（自定义事件）
 */

import { IEventConfigLike } from "./MiddleablePostbar";

/**
 * 自定义事件配置（参照 TRANSTRIAL_ACCESSING 格式配置）
 * @description 格式如下：
 * <事件名>: {
 *     <埋点类型 1>: { <埋点数据> },
 *     <埋点类型 2>: { <埋点数据> },
